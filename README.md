...
- **Annual Premiums Collected by Municipalities:** Display the total annual premium revenue generated from each municipality/cities.
- **Average Premium per Policy by Municipalities:** Shows the average premium amount paid per policy in each municipality/cities.
- **Policy Distribution by Munipalities and Type:** Illustrates the number of policies for each insurance type across municipalities/cities.
- **Policies Sold per Agent:** Compares the number of insurance policies sold by each agent.
- **Policies Status Overview:** Summarizes the distribution of policies based on their current status (Active, Pending, Expired and Cancelled).
## Instructions for Use
1. Download the dataset (*i.e. Home Insurance Data Analysis Practice.xlsx*) from the provided Github repository link.
2. Explore the interactive dashboard to gain insights.
3. Modify the project as needed for your specific requirements.
